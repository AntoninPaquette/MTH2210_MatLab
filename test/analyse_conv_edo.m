...
for t=1:nb_eval
	...
	erreur_euler_exp(t)	=	norm(y_euler_exp - sol_exacte(temps),1);
	[temps , y_milieu]		=	pt_milieu(f , tspan , x0 , nb_pas(t));
	erreur_milieu(t)		=	norm(y_milieu - sol_exacte(temps),1);
	[temps , y_euler_mod]	=	euler_mod(f , tspan , x0 , nb_pas(t));
	erreur_euler_mod(t)	=	norm(y_euler_mod - sol_exacte(temps),1);
	[temps , y_rk4]			=	rk4(f , tspan , x0 , nb_pas(t));
	erreur_rk4(t)			=	norm(y_rk4 - sol_exacte(temps),1);
	[temps , y_euler_imp]	=	euler_imp(f , tspan , x0 , nb_pas(t));
	erreur_euler_imp(t)	=	norm(y_euler_imp - sol_exacte(temps),1);
	[temps , y_crank_nic]	=	crank_nic(f , tspan , x0 , nb_pas(t));
	erreur_crank_nic(t)	=	norm(y_crank_nic - sol_exacte(temps),1);
end
